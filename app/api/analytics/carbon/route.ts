...
import { estimateCarbon } from "../../../../lib/ml/carbon"

export async function GET() {
  const supabase = await createClient()
  const { data: rentals } = await supabase
    .from("rentals")
    .select("id, equipment_id, check_out_engine_hours, check_in_engine_hours, status")
  const { data: equipment } = await supabase
    .from("equipment")
    .select("id, equipment_id, engine_hours")
  ...
  const usage = (equipment || []).map((e: any) => {
    ...
    const fuelEfficiency = 3 + (Math.random() - 0.5)
    return {
  ...
  const carbon = estimateCarbon(usage)
